...
pub struct TodoRequeue;

#[async_trait]
impl EngineTool for TodoRequeue {
    fn name(&self) -> &str {
        "TodoRequeue"
    }
    ...
    async fn invoke(&self, input: Value, ctx: &RunnerContext) -> Result<ToolOutput, AoError> {
        ...
        match svc
            .requeue_task_for_agent(&ctx.agent_id, &active.id, &task_id)
            .await
        {
            Ok(()) => Ok(ToolOutput::text(format!(
                "task '{}' reset to pending — feeder will re-dispatch on next advance",
                task_id
            ))),
            ...
            Err(e) => Ok(ToolOutput::error(
                &format!("failed to requeue task: {e}"),
                false,
            )),
        }
    }
