...
pub struct TodoStopTask;

#[async_trait]
impl EngineTool for TodoStopTask {
    fn name(&self) -> &str {
        "TodoStopTask"
    }
    ...
    async fn invoke(&self, input: Value, ctx: &RunnerContext) -> Result<ToolOutput, AoError> {
        ...
        match svc
            .stop_task_for_agent(&ctx.agent_id, &active.id, &task_id)
            .await
        {
            Ok(()) => Ok(ToolOutput::text(format!(
                "task '{}' stopped — use TodoResumeTask to re-queue it when ready",
                task_id
            ))),
