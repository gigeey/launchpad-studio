...
pub struct WorkflowActionDelete;

#[async_trait]
impl IoTool for WorkflowActionDelete {
    fn name(&self) -> &str {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "task_id": {
                    "type": "string",
                    "description": "ID of the workflow task to delete."
                }
            },
            "required": ["task_id"],
    ...
    async fn invoke(&self, input: Value, ctx: &RunnerContext) -> Result<ToolOutput, AoError> {
        ...
        match runner.delete_task(&task_id).await {
