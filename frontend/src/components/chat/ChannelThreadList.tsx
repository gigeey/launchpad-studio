...
import { ChevronRight, Hash, Inbox, X } from "lucide-react";
import { CHANNEL_KIND_ICON } from "./ThreadTabStrip";
import { CHANNEL_KIND_LABELS } from "../../lib/threadNavigation";
import { ThreadUnreadDot } from "../shared/ThreadActivityBadge";
...
interface ChannelThreadListProps {
   ...
   *  `ChannelsColumn`'s own, identically-shaped, state). */
   ...
   *  close button. Once archived, `resolveChannelThreadPartition` drops the
   *  thread from `channelGroups` entirely, so the row disappears from this
   *  list on the next render. */
  onArchiveThread: (threadId: string) => void;
}

/** Shared "browse channel conversations" list — one group header per
 *  `channel_origin.kind` plus its rows, an empty state when there are no
 *  channel conversations yet. Used both by `ChannelsTilePanel` (the
 *  Channels tile's floating popover) and `ChannelsColumn` (the persistent,
 *  pinned version of the same list) so the two surfaces render identical
 *  content — pinning/unpinning only changes the surrounding chrome, never
 *  the list itself. Reuses the `--modal-*` color tokens in both places;
 *  those resolve to the exact same values as their base `--text-*`/`--bg-*`
 *  counterparts (see App.css), so this reads correctly whether it's sitting
 *  in a floating popover or embedded directly in the page. */
export function ChannelThreadList({
  ...
  // Per-kind collapse — local rather than persisted, since this list is
  // reused by both the floating popover and the pinned column and a fresh
  // "everything expanded" state on each mount is the least surprising
  // default for either surface.
  const [collapsedKinds, setCollapsedKinds] = useState<Set<ChannelThreadGroup["kind"]>>(new Set());
  const toggleGroup = (kind: ChannelThreadGroup["kind"]) => {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain pb-[4px]">
      {channelGroups.map((group) => {
        const GroupIcon = CHANNEL_KIND_ICON[group.kind];
        const isCollapsed = collapsedKinds.has(group.kind);
        return (
          <div key={group.kind} data-testid={`channels-tile-group-${group.kind}`}>
            <div
              role="button"
              tabIndex={0}
              ...
              data-testid={`channels-tile-group-toggle-${group.kind}`}
              className="group/hdr flex items-center gap-[6px] px-[12px] pt-[10px] pb-[4px] cursor-pointer select-none bg-[var(--modal-bg-tertiary)] border-b border-[var(--modal-border-secondary)]"
            >
              {/* Section header shows the channel-kind glyph at rest and
                  morphs it into the collapse chevron on hover/focus — never
                  both at once. Both share one fixed-size slot so swapping
                  them never nudges the label. */}
              <span className="relative flex items-center justify-center w-[12px] h-[12px] shrink-0">
                <GroupIcon className="w-[11px] h-[11px] text-[var(--modal-text-tertiary)] transition-opacity group-hover/hdr:opacity-0 group-focus-visible/hdr:opacity-0" />
                <ChevronRight
              ...
              <span className="text-[11px] font-bold uppercase tracking-wide text-[var(--modal-text-primary)]">
                {CHANNEL_KIND_LABELS[group.kind]}
              </span>
            </div>
              ...
              return (
                ...
                >
                  <button
                    ...
                    data-testid={`channels-tile-row-${info.thread.id}`}
                    title={info.label}
                  ...
                  <button
                    ...
                    data-testid={`channels-tile-archive-${info.thread.id}`}
